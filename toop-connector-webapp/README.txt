toop-connector-webapp docker image build and run scripts
...
2. To run the image and create a container with default values 
    ...
    This command will use  parameters jdk_volume=jdk_home, toop_dir=toop-dir,  external_port=8080 as default. 
        ...
        external_port=8081: The container will publish its internal default connector port (which is also 8080)
        to the localhost (the HOST machine) on port 8081.
...
3. To run the image and create a container with custom values
    ...
    the ./run.sh script will create a container with name "toop-connector-webapp". You can also parameterize it to create multiple 
    containers from the same image.

4. to stop the container
    docker stop toop-connector-webapp
    
    
Other files:
1. connector-setup.sh: This is used during the build stage. It downloads and installs the connector war file into
...
and creates the toop-dir and fills the sub directories with the config files and creates the whole /toop-dir/tc 
folder structure and then runs 'catalina.sh run'.
